...
typealias VideoFrame = CVPixelBuffer
...
class VideoReader {
    ...
    init?(videoAsset: AVAsset) {
        ...
        self.videoTrack = array.first
        print("create videoReader")
    ...
    func nextFrame() -> CGImage? {
        ...
        let tmpbuffer = CMSampleBufferGetImageBuffer(sampleBuffer)!
        let image = CIImage(cvImageBuffer: tmpbuffer)
